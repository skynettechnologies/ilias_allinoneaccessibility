...
class ilAllinoneaccessibilityConfigGUI extends ilPluginConfigGUI {
    ...
    // configuration of module - set iFrame HTML
    protected function configure($htmlMessage = ""): void
    {
        ...
        if(isset($settingURLObject->status) && $settingURLObject->status == 3){
            ...
            $iframe .= '<div style="width:80%; margin-top: 50px; padding-bottom: 10px;display:flex;justify-content:space-between;">';
            ...
            $iframe .= '</div>';
            $iframe .= '<iframe id="aioamyIframe" width="80%" style="max-width: 1920px; border: none;" height="1100px"  src="'.$settingURLObject->settinglink.'"></iframe>';
        }else{
            $iframe = '<iframe src="https://ada.skynettechnologies.us/trial-subscription?isframe=true&platform=jumpseller&website='.$aioa_website_hostname.' " height="1100px;" width="80%" style="border: none;"></iframe>';
        }
        // set Content - iFrame HTML
